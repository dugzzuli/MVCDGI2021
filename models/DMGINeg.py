...
class DMGINeg(embedder):
    ...
    def training(self,f):
        ...
        Dadj = [adj_.to(self.args.device) for adj_ in self.Dadj]
        ...
        for epoch in iters:
            ...
            result = model(features, adj,Dadj, shuf, self.args.sparse, None, None, None)
            ...
            if(epoch%10)==0:
                ...
                nmi,acc,stdacc,stdnmi=evaluate(model.H.data.detach(), self.idx_train, self.labels, self.args.device)
                if(accMax<acc):
                    accMax=acc
                    nmiMax=nmi
                    curepoch=epoch
                iters.set_description("epoch:{} acc:{} nmi:{} accMax:{} nmiMax:{} curepoch:{}".format(epoch,acc,nmi,accMax,nmiMax,curepoch))
...
class modeler(nn.Module):
    ...
    def forward(self, feature, adj,Dadj, shuf, sparse, msk, samp_bias1, samp_bias2):
        ...
        for i in range(self.args.nb_graphs):
            ...
            h_2 = self.gcn[i](shuf[i], Dadj[i], sparse)
        ...
        if self.args.isAttn:
            ...
            if (self.args.isMeanOrCat == 'Mean'):
                ...
                h_1_all = torch.cat(h_1_all,2).squeeze()
                h_2_all = torch.cat(h_2_all,2).squeeze()
