...
from layers import GCN, Discriminator, AttentionNoALL
import numpy as np
np.random.seed(0)
...
class modeler(nn.Module):
    def __init__(self, args):
        ...
        if(self.args.isMeanOrCat=='Mean'):
            ...
            self.disc=Discriminator(args.hid_units* self.args.View_num)
            
        self.readout_func = self.args.readout_func
        if args.isAttn:
            self.attn = nn.ModuleList([AttentionNoALL(args) for _ in range(args.nheads)])
    ...
    def forward(self, feature, adj, shuf, sparse, msk, samp_bias1, samp_bias2):
        ...
        if self.args.isAttn:
            ...
            for h_idx in range(self.args.nheads):
                h_1_all_, h_2_all_ = self.attn[h_idx](h_1_all, h_2_all)
        ...
        c = self.readout_func(h_1_all)
        c = self.args.readout_act_func(c)  # equation 9
        
        logit = self.disc(c, h_1_all, h_2_all, samp_bias1, samp_bias2)
