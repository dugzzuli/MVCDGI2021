...
from layers import GAT, Discriminator, Attention
import numpy as np
np.random.seed(0)
...
class DMGIGAT(embedder):
    ...
    def training(self,f):
        ...
        for epoch in iters:
            ...
            if(epoch%10)==0:
                ...
                if(accMax<acc):
                    ...
                    arimax=ari
                    curepoch=epoch
                retxt="epoch:{} acc:{} nmi:{} accMax:{} nmiMax:{} ariMax:{} curepoch:{}".format(epoch,acc,nmi,accMax,nmiMax,arimax,curepoch)
        ...
        model.eval()
        nmi,acc,ari,stdacc,stdnmi,stdari=evaluate(model.H.data.detach(), self.idx_train, self.labels, self.args.device)
        return nmi,acc,stdacc,stdnmi,retxt
...
class modeler(nn.Module):
    def __init__(self, args):
        ...
        self.gcn = nn.ModuleList([GAT(hid, args.hid_units, 0.2) for _,hid in zip(range(args.nb_graphs),self.args.dims)])
