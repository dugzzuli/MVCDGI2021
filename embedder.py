...
class embedder:
    def __init__(self, args):
        ...
        if(args.rownetworks!=None):
            adj, features, labels, idx_train=args.rownetworks, args.truefeatures_list, args.labels, args.idx_train
            print("no reading ")
        else:
            adj, features, labels, idx_train = process.load_data_mv(args)
        dims=[np.shape(dim)[1] for dim in features]
